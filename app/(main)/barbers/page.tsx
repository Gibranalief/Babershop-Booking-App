  ...
  return (
    <main className="pt-28 pb-32 max-w-7xl mx-auto px-6 md:px-12">
      <header className="mb-16">
        <p className="text-primary font-label text-sm uppercase tracking-[0.3em] mb-4">
          The Masters
        </p>
      ...
      {isLoading && (
        <div className="flex flex-col items-center justify-center py-32 gap-4">
          ...
          <p className="text-outline text-sm font-label uppercase tracking-widest">
            Loading barbers...
          </p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
          {barbers.map((barber) => (
            <Link
              ...
              className="group flex flex-col relative w-full translate-y-0 lg:translate-y-0"
            >
              <div className="relative aspect-[3/4] mb-6 overflow-hidden asymmetric-radius bg-surface-container shadow-2xl">
                ...
                <img
                  src={
                    barber.imageUrl ||
                    "https://lh3.googleusercontent.com/aida-public/AB6AXuBKIxvNJLGg8F-EPq8pvN72BjJlKgDHH1_92roYJSO5MwxxsRfwxzgQUGeW2p7vWxpDY2BGwWnJwpnBg8xKJIdNN20OFwlComnVDsMclKLi08MyHkoR4Sv7_YCAtJ4qYI-tdsvKgM5RZEWZazvl6BFNTm11JZAj1pYgDpDfExxzENFN4XcgexAkeeUedN1yfxRJcPsAey9Rw0V_LDqGyIvXl3FDmtLd3X4aaYBrDpaBvSu0vFmV2HT_7l3PHj4gWAUSXO8ariUzjMg"
                  }
                ...
                {barber.specialty && (
                  <div className="absolute bottom-4 left-4 z-20 flex gap-2">
                    {barber.specialty.split(",").map((tag, i) => (
                      <span
                        key={i}
                        className="bg-surface-container-highest/80 backdrop-blur-md text-on-surface px-3 py-1 rounded-full text-[10px] uppercase tracking-widest font-bold font-label border border-outline-variant/20"
                      >
                        {tag.trim()}
                      </span>
                    ))}
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-3xl font-black font-headline mb-1 group-hover:text-primary transition-colors">
                    {barber.user.name}
                  </h3>
                  <p className="text-outline text-sm font-label uppercase tracking-widest">
